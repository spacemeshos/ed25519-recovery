...
import "github.com/spacemeshos/ed25519-recovery/internal/edwards25519"

// InvertModL computes 1/x mod l and puts the result into s
func InvertModL(x *edwards25519.Scalar) *edwards25519.Scalar {
	// Outline the function body so that the returned Scalar can be
	// stack-allocated.
...
func invertModL(s, x *edwards25519.Scalar) {

	t1 := edwards25519.NewScalar().Set(x)          // 2^0
	t0 := edwards25519.NewScalar().Multiply(x, x)  // 2^1
	t2 := edwards25519.NewScalar().Multiply(t0, x) // 2^1 + 2^0
	for i := 1; i < 2; i++ {                       // 2^2
		t0.Multiply(t0, t0)
	}
	t3 := edwards25519.NewScalar().Multiply(t0, t2) // 2^2 + 2^1 + 2^0
	for i := 1; i < 2; i++ {                        // 2^3
		t0.Multiply(t0, t0)
	}
	t4 := edwards25519.NewScalar().Multiply(t0, t3) // 2^3 + 2^2 + 2^1 + 2^0
	for i := 1; i < 2; i++ {                        // 2^4
		t0.Multiply(t0, t0)
	}
	t5 := edwards25519.NewScalar().Multiply(t0, t4) // 2^4 + 2^3 + 2^2 + 2^1 + 2^0

	tz := edwards25519.NewScalar().Set(x) // 2^252
