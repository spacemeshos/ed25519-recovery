...
func BenchmarkInvertModL(b *testing.B) {
	xBytes := make([]byte, 32)
	xBytes[0] = byte(2)
	x, err := edwards25519.NewScalar().SetBytesWithClamping(xBytes)
	require.NoError(b, err, "failed to set bytes")

	xInv := edwards25519.NewScalar()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		xInv = InvertModL(x)
	}
...
func TestInvertModLOne(t *testing.T) {
	xBytes := make([]byte, 32)
	xBytes[0] = byte(1)
	x, err := edwards25519.NewScalar().SetCanonicalBytes(xBytes)
...
func TestInvertModLRnd(testing *testing.T) {
	for i := 1; i < 100; i++ {
		t, err := edwards25519.NewScalar().SetUniformBytes(rnd64Bytes(testing))
		require.NoError(testing, err, "failed to set bytes")

		tinv := InvertModL(t)
		out := edwards25519.NewScalar().Multiply(t, tinv)
